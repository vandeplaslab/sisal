...
def normalize_train_test_full_loader(centroids: np.ndarray, mask: np.ndarray, batch_size: int = 32, alpha=None):
    ...
    """
    return normalize_train_test_full_loader_given_index(
        centroids, np.array(range(centroids.shape[0])), mask, batch_size, alpha
    )


def normalize_train_test_full_loader_given_index(
    centroids: np.ndarray, pixel_index: np.ndarray, mask: np.ndarray, batch_size: int = 32, alpha=None
):
    """
    Normalize the given data and return the train, test, and full data loaders.
    ...
    if alpha is None:
        full_loader = loader = torch.utils.data.DataLoader(
            [[val[i], mask[i], index[i]] for i in range(val.shape[0])],
            shuffle=False,
            batch_size=batch_size,
            pin_memory=True,
            drop_last=True,
        ...
        full_loader = loader = torch.utils.data.DataLoader(
            [[val[i], mask[i], alpha[i], index[i]] for i in range(val.shape[0])],
            shuffle=False,
            batch_size=batch_size,
            pin_memory=True,
            drop_last=True,
