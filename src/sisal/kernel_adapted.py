...
def _compute_density_corrected(grid_x: np.ndarray, grid_y: np.ndarray, latents_1: np.ndarray, latents_2: np.ndarray, vars_0: np.ndarray, vars_1: np.ndarray, h: float, n: int) -> np.ndarray:
    ...
    for p in prange(n_pixels):
        ...
        for i in range(n):
            ...
            dist_sq = (gx - latents_1[i]) ** 2 / vars_0[i] + \
                      (gy - latents_2[i]) ** 2 / vars_1[i]
