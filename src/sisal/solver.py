...
class Solver():
    ...
    def train_one_epoch(self,epoch_index,dataloader,tb_writer):
        c=0
        ...
        for i,(x,_) in enumerate(dataloader) :
                ...
                if i%2000 == 1999 :
                    ...
                    if self.save_epochs:
                        torch.save(self.model, 'model/model_weight_n_{}.pth'.format(epoch_index*3+c))
                        c+=1

                    
                    # if self.evaluate_dis_metric :
                    #     with torch.no_grad():
                    #         full_latent= compute_latent_mean(dataloader,self.model)
                    #         full_std = emp_std(full_latent)
                    #         z_min, z_max = limit_latent_space_precomp(full_latent)
                    #         dis_metric = metric_disentangling(self.model,z_min,z_max,full_std)    
                    #         tb_writer.add_scalar('Metric/disentangling', dis_metric, tb_x)
                    #         tb_writer.add_scalar('Metric/reconstruction', running_recons, tb_x)
                    # running_recons=0.
    ...
    def train(self,train_loader, validation_loader, PATH , v=0):
        ...
        if self.train_bool :
            ...
            results = []
            if self.save_loss : 
                train_loss , train_recons , train_kl, test_loss = self.evaluate_initialization(train_loader,validation_loader)
            ...
            for epoch in range(self.EPOCHS) :
                if early_stop >= epoch_early_stop : 
                    ...
                    if self.save_loss :
                        print('#### saved_data/avg_models/model_z{}_b{}_v{}.npy'.format(self.z_dim,self.beta,v))
                        with open('saved_data/avg_models/model_z{}_b{}_v{}.npy'.format(self.z_dim,self.beta,v), 'wb') as f:
                            np.save(f, results)
                    return PATH
                ...
                avg_loss, avg_recons, avg_KL= self.train_one_epoch(epoch,train_loader,self.writer)
                if self.save_epochs :
                    torch.save(self.model, 'model/model_weight_n_{}.pth'.format(epoch*3+2))
...
def train_batch_models(args ,train_loader,test_loader):
    ...
    for r in range(n_rep) :
        PATH = 'model/average_models/model_weights_z{}_b{}_v{}.pth'.format(args.z_dim,args.beta,r)
        print(PATH)
        net = Solver(args)
...
def train_batch_beta(args,in_size, train_loader, test_loader):
    args.train = True
    args.zdim = 2
    PATH = 'model/average_models/mouse_pup/model_b_{}.pth'.format(args.beta)
    print(PATH)
